...
def read_input(raw: str) -> (dict, list):
    rules = dict()
    messages = list()
...
def check(s: str, seq, rules) -> bool:
    if s == "" or seq == []:
        return s == "" and seq == []
    ...
    if type(r) == str:
        if s[0] in r: 
            return check(s[1:], seq[1:], rules)
        else:
            return False
    else:
        return any(check(s, t + seq[1:], rules) for t in r)

def ans1(rules, messages):
    ans = 0
    for m in messages:
        if check(m, ['0'], rules):
            ans += 1
...
rules, messages = read_input(TEST)
print(ans1(rules, messages))

REAL = '''94: 118 64 | 22 34
21: 16 64 | 49 34
70: 58 34 | 106 64
100: 58 64 | 56 34
24: 29 64 | 128 34
...
rules, messages = read_input(REAL)
print(ans1(rules, messages))
